...
describe 'Summary columns Routes' do
  ...
  it '(SAD) should return error offset greater or equal than total' do
    get "/api/v0.1/tables/#{TABLE_VALID}/summary?column=#{COLUMN_VALID}&limit=#{LIMIT}&offset=#{TOTAL}"
    last_response.status.must_equal 400
    last_response.body.must_include 'offset'

    get "/api/v0.1/tables/#{TABLE_VALID}/summary?column=#{COLUMN_VALID}&limit=#{LIMIT}&offset=#{TOTAL+1}"
    last_response.status.must_equal 400
    last_response.body.must_include 'offset'
  end
